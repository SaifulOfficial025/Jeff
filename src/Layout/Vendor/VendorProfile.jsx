...
import { useGetProfileQuery, useUpdateVendorProfileMutation } from '../../redux/features/baseApi';
import toast, { Toaster } from 'react-hot-toast';

const VendorProfile = () => {
  const { data: profile, isLoading: profileLoading, error } = useGetProfileQuery();
  const [updateVendorProfile, { isLoading: updating }] = useUpdateVendorProfileMutation();
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      await updateVendorProfile(profileData).unwrap();
        ...
        <div className="relative bg-gray-800 p-6 rounded-lg shadow-xl mx-auto max-w-4xl">
          <div className="flex flex-col md:flex-row items-center md:items-start">
            ...
            <div className="relative w-32 h-32 md:w-40 md:h-40 border-4 border-gray-700 shadow-lg md:mr-6">
              <img
                src={profile?.profile_image || "https://placehold.co/160x160/0A0A0A/FFFFFF?text=User"}
                alt="Profile Avatar"
            ...
            <div className="flex-grow mt-4 md:mt-0 text-center md:text-left">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h2 className="text-3xl font-bold text-gray-50">{firstName} {lastName || profile?.company_name || profile?.email || 'User'}</h2>
